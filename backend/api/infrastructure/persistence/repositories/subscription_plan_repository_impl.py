...
class SubscriptionPlanRepository(BaseRepository[SubscriptionPlan]):
    ...
    async def create_subscription_plan(self, subscription_plan: SubscriptionPlan) -> PydanticObjectId | None:
        result = await super().create(subscription_plan.model_dump(exclude_none=True))
        return result.id
